...
首页改为科普导览：三句话介绍 Jev、六个精选案例、11 个可展开分类和简短收录说明。保留全部 112 项的简介、媒体、证据链接与内容更新日期；精确时间、技术细节和 A/B/C 报告留在详情。新增双语首页编排数据与统一渲染器。本次为呈现改版，不新增案例、不改变已有证据判断与内容时间。
...
将已完成的 106 项审核标到中英文 README、案例索引和详情页：**A 18 项、B 78 项、C 10 项**。每项有等级、简短理由及直接指向[审核报告](references/2026-09-19-claims-audit.md)对应行的证据链接。等级评价公开主张，不改变复现状态。

依据审核完成于北京时间 11:30；本次发布标记更新内容时间，保留首次收录时间、主帖快照和媒体。未增删案例。审核元数据与双语理由纳入目录生成，未来未审核的新条目明确显示中性标记。
