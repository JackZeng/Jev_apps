...
**Collection snapshot: {date} (Asia/Shanghai).** This is a collection from that search, not an exhaustive inventory of X. All examples are **not independently reproduced**. Performance and cost figures are attributed to their authors; comparisons are analysis of public designs, not our own benchmarks.
...
| Collected / record updated | {date} / {date} |
...
| Last source review | {date}; public descriptions and metadata reviewed, application not run |
...
| --- | --- | --- | --- |
| {c['reported_result']} | Author report | [Post and attached media]({p['url']}) | Not reproduced here; a demo does not establish general performance |
