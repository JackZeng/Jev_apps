...
The homepage is a field guide: six expanded highlights followed by a collapsible complete catalog. `data/homepage.json` stores featured slugs, bilingual short explanations and category introductions without copying sources or dates. Select highlights for clear uses and mechanism evidence, not likes alone. Standard cards use catalog summaries; detailed mechanisms, exact snapshots and A/B/C assessments remain in the records. Homepage labels use plain language and show only the Beijing content-update date. Layout changes do not refresh historical dates; changed facts or judgments must first be reflected in catalog content and its update timestamp.
...
Display only **Content updated** beside each application, with no first-added or per-case assessment timestamp. Preserve `readme_added_at`, source snapshots and audit timestamps in the data and evidence records. Formatting-only changes do not advance content timestamps.
